---
title: "Sea Cucumber Differential Gene Expression Data of Intestine Regenerative Tissue"
...
names <- read.table("genenames.txt", header=FALSE, row.names = 1)
ddata <- read.csv("ddata.csv", header = TRUE)
...
```{r}
ddsSTAR <- DESeqDataSetFromHTSeqCount(sampleTable = ddata, directory = "countsmay2019", design = ~ condition)
...
Estimating the size factors using "median ratio method", dispersions (gene-wise, mean-dispersion relationship, final dispersion estimates). Generating the counts table with _counts_ function.
...
Restults of contrasts between conditions (3DPE vs Normal; 1DPE vs Normal). __Note: Normal tissue is referred to non-regenerated intestine tissue of the sea cucumber.__
...
write.csv(as.data.frame(res_3DPE), file="dqData2019_3DPE.csv")
```
...
EnhancedVolcano(res_3DPE,
                ...
                x = 'log2FoldChange',
                y = 'padj',
                selectLab = c('TRINITY_DN42961_c0_g1_i1'),
                xlim = c(-14,14),
